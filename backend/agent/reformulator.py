...
Generate a targeted retrieval query for the next turn. Return ONLY a valid JSON object."""
    ...
    def _parse_reformulation(
        ...
        try:
            ...
            if ref_query and isinstance(ref_query, str) and ref_query.strip():
                # Clean any artifact prefixes if model regurgitated them
                cleaned_ref = re.sub(r"(?i)no evidence retrieved for query:\s*", "", ref_query).strip(" '\"")
                return {
                    "reformulated_query": cleaned_ref or original_query,
                    "reasoning": parsed.get("reasoning", "Generated by QueryReformulator."),
                    "suggested_tool": parsed.get("suggested_tool") or evaluation.recommended_tool or "hybrid_search",
                }
    ...
    def _fallback_reformulation(
        ...
        if evaluation.missing_information:
            raw_gap = " ".join(evaluation.missing_information)
            cleaned_gap = re.sub(r"(?i)no evidence retrieved for query:\s*", "", raw_gap).strip(" '\"")
            fallback_query = cleaned_gap if cleaned_gap else original_query
        else:
        ...
        return {
            ...
            "suggested_tool": evaluation.recommended_tool or "hybrid_search",
        }
